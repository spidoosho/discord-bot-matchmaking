...
module.exports = {
	...
	async execute(interaction) {
		...
		if (!interaction.member.roles.cache.has(superAdminRole.id)) {
			return interaction.reply({ content: `Only <@&${superAdminRole.id}> can remove <@&${adminRole.id}> role.`, ephemeral: true });
		}

		const user = interaction.options.getUser('user');
		const member = interaction.guild.members.cache.find(target => target.id === user.id);
		if (!member.roles.cache.has(adminRole.id)) {
			return interaction.reply({ content: `Member <@${member.id}> is not a <@&${adminRole.id}>.`, ephemeral: true });
		}

		member.roles.remove(adminRole);
		return interaction.reply({ content: `Member <@${member.id}> is no longer a <@&${adminRole.id}>.`, ephemeral: true });
	},
};
